...
def batch_export_asrc(PLATFORM):
    ...
    for path, subdirs,files in os.walk(root):
        for file in files:
            ...
            if (ext in ['zhcn','zhtw'] and os.path.isfile(f'{truepath}.ja') and ('.asrc' in file)) or (f'.asrc' in file and ext in file) or ('.asrc' in file and ext == 'all'): #chinese sounds don't have any extension, since they are the same for zhcn and zhtw
                try:
